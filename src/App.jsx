...
function Cadastrar({ onSalvar, onVoltar }) {
  ...
  const onFoto = (e) => {
    ...
    r.onload = async (ev) => { const compressed = await compressImage(ev.target.result); setFoto(compressed); await analisarIA(compressed.split(",")[1], f.type); };
    r.readAsDataURL(f);
  };
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:200,display:"flex",alignItems:"flex-end"}}>
          <div style={{background:"#fff",borderRadius:"20px 20px 0 0",padding:20,width:"100%",boxSizing:"border-box",maxHeight:"90vh",overflowY:"auto"}}>
            <input ref={fileRef} type="file" accept="image/*" capture="environment" style={{display:"none"}}
              onChange={e=>{const f=e.target.files?.[0];if(f){const r=new FileReader();r.onload=async ev=>{const compressed = await compressImage(ev.target.result);setFotoAs(compressed);};r.readAsDataURL(f);}}} />
